/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ResolvedTicketsWhereInput } from "./ResolvedTicketsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ResolvedTicketsOrderByInput } from "./ResolvedTicketsOrderByInput";

@ArgsType()
class ResolvedTicketsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ResolvedTicketsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ResolvedTicketsWhereInput, { nullable: true })
  @Type(() => ResolvedTicketsWhereInput)
  where?: ResolvedTicketsWhereInput;

  @ApiProperty({
    required: false,
    type: [ResolvedTicketsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ResolvedTicketsOrderByInput], { nullable: true })
  @Type(() => ResolvedTicketsOrderByInput)
  orderBy?: Array<ResolvedTicketsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ResolvedTicketsFindManyArgs as ResolvedTicketsFindManyArgs };
